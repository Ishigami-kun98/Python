...
toT = {'F' : toF, 'K': toK, 'R':toR, 'De': toDe, 'N':toN, 'Ré': toRé, 'Ro': toRo, 'C':toC}
fromT = {'F' : fromF, 'K': fromK, 'R':fromR, 'De': fromDe, 'N':fromN, 'Ré': fromRé, 'Ro': fromRo, 'C':fromC}

def fromTtoAll(n, t):
    celsius = fromT[t](n)
    return {scale : covert(celsius) for scale, convert in toT.items()}

def table(n):
    tot = [
        [v for k, v in sorted(fromTtoAll(n, t).items(), key=lambda x:x[0])
